...
contract MultiSigDeleGator is DeleGatorCore {
    ...
    bytes32 private constant DELEGATOR_STORAGE_LOCATION =
        0xb005e320c74f68de39b3d9025549122b8b117c48474f537aac49c12147b61c00;
    ...
    function replaceSigner(
        ...
        if (_newSigner == address(0) || _newSigner.code.length != 0)
    ...
    function _setSignersAndThreshold(
        ...
        uint256 signersCount_ = _clearSigners
            ? signersLength_
            : signersLength_ + storedSignersLength_;
        if (_threshold == 0 || _threshold > signersCount_)
        ...
        for (uint256 i = 0; i < signersLength_; ++i) {
            ...
            if (newSigner_ == address(0) || newSigner_.code.length != 0)
                revert InvalidSignerAddress();
    ...
    function _isValidSignature(
        ...
        if (_signatures.length != s_.threshold * SIGNATURE_LENGTH)
        ...
        for (uint256 i = 0; i < signatureCount_; ++i) {
            bytes memory signature_ = _signatures[i * SIGNATURE_LENGTH:(i + 1) *
                SIGNATURE_LENGTH];

            currentOwner_ = ECDSA.recover(_hash, signature_);

            if (currentOwner_ <= lastOwner_ || !s_.isSigner[currentOwner_])
                return ERC1271Lib.SIG_VALIDATION_FAILED;
    ...
    function _getDeleGatorStorage()
        internal
        pure
        returns (MultiSigDeleGatorStorage storage s_)
    {
        assembly {
