...
contract HybridDeleGator is DeleGatorCore, IERC173 {
    ...
    bytes32 private constant DELEGATOR_STORAGE_LOCATION =
        0xa2b1bcb5e16cee2a8898b49cb0c3605e70c16f429f6002ed8b1bc5612a694900;
    ...
    error InputLengthsMismatch(
        uint256 keyIdsLength,
        uint256 xValuesLength,
        uint256 yValuesLength
    );
    ...
    ) DeleGatorCore(_delegationManager, _entryPoint) {}
    ...
    ) external reinitializer(_version) onlyEntryPointOrSelf {
    ...
    function removeKey(string calldata _keyId) external onlyEntryPointOrSelf {
        ...
        if (keyIdHashesCount_ == 1 && s_.owner == address(0))
    ...
    function getKey(
        string calldata _keyId
    ) external view returns (uint256 x_, uint256 y_) {
        HybridDeleGatorStorage storage s_ = _getDeleGatorStorage();
        P256PublicKey memory publicKey_ = s_.authorizedKeys[
    ...
    function transferOwnership(
        address _newOwner
    ) external virtual onlyEntryPointOrSelf {
    ...
    function _addKey(string calldata _keyId, uint256 _x, uint256 _y) internal {
        if (!P256FCLVerifierLib.isValidPublicKey(_x, _y))
            revert KeyNotOnCurve(_x, _y);
        bytes32 keyIdHash_ = keccak256(abi.encodePacked(_keyId));
        ...
        if (publicKey_.x != 0 || publicKey_.y != 0)
            revert KeyAlreadyExists(keyIdHash_);
    ...
    function _transferOwnership(address _newOwner) internal {
        HybridDeleGatorStorage storage s_ = _getDeleGatorStorage();
        if (s_.keyIdHashes.length == 0 && _newOwner == address(0))
            revert CannotRemoveLastSigner();
    ...
    function _updateSigners(
        ...
        if (_owner == address(0) && keysLength_ == 0 && _deleteP256Keys)
            revert SignersCannotBeEmpty();
        ...
        if (_deleteP256Keys) {
            ...
            if (keyIdHashesCount_ != 0) {
                for (uint256 i = 0; i < keyIdHashesCount_; ++i) {
                    ...
                    P256PublicKey memory pubKey_ = s_.authorizedKeys[
                        keyIdHash_
                    ];
                    delete s_.authorizedKeys[keyIdHash_];
    ...
    function _isValidSignature(
        ...
        if (sigLength_ == 65) {
            ...
            if (key_.x == 0 && key_.y == 0)
                return ERC1271Lib.SIG_VALIDATION_FAILED;

            if (
                sigLength_ == 96 &&
                P256VerifierLib._verifyRawP256Signature(
                    _hash,
                    _signature,
                    key_.x,
                    key_.y
     ...
     */
    function _getDeleGatorStorage()
        internal
        pure
        returns (HybridDeleGatorStorage storage s_)
    {
        assembly {
